...
2. Candidate identity, before Phase 3. `sameCandidate()` merges "Ciro Nogueira" into
   ...
   **The worklist is written up in `REVISAO_CANDIDATOS.md`**, regenerated by
   `node scripts/candidate-review.mjs` — 63 pairs with evidence (party trajectory, twin
   fieldwork, institutes, period, magnitude), sorted into five buckets, each awaiting a
   tick. Two things it teaches that the pairs alone don't: the "twin fieldwork" signal
